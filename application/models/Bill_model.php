...
class Bill_model extends CI_model {
    ...
    function getTotalCreateBill($p_idd) {
        $this->db->select_sum('create_bill_taka');
        $this->db->where('patient_iid', $p_idd);
        $query = $this->db->get('create_indoor_bill');
        return $query->row()->create_bill_taka;
    }
    ...
    function getadvncBill($p_id) {
        $this->db->where('ptn_iid', $p_id);
        $query = $this->db->get('bill_advnc');
        return $query->result();
    }

    function get_create_bill_for_receives($p_idd) {
        $this->db->where('patient_iid', $p_idd);
        $this->db->where('create_bill_taka !=', '0');
        $this->db->join('bill_cat', 'create_indoor_bill.bill_cat_iid = bill_cat.c_num');
        $query = $this->db->get('create_indoor_bill');
        return $query->result();
    }


    function getadvtk($p_id) {
        $this->db->where('ptn_iid', $p_id);
        $query = $this->db->get('bill_advnc');
        return $query->result();
    }
    ...
    function getIndoorBill_Update($p_id) {
        $this->db->order_by('c_num');
        $this->db->where('patient_iid', $p_id);
        $this->db->join('bill_cat', 'bill_cat.c_num = create_indoor_bill.bill_cat_iid');
        $query = $this->db->get('create_indoor_bill');
        return $query->result();
    }
    ...
    function getPatientById($id) {
        ...
        $this->db->join('nurse', 'nurse.n_id = patient.bll_rcv_emp', 'left');
        $this->db->join('doctor', 'doctor.dr_auto_id = patient.dr_a_iniq_idd', 'left');
        $query = $this->db->get('patient');
        return $query->row();
    }
   ...
   function getbed_taka($bed_no) {
    ...
    function billForStatmnt($qu_date) {
     ...
     function patientForStatemnt($qu_date) {
         $this->db->where('brdate', $qu_date);
         $this->db->join('doctor', 'patient.dr_id = doctor.dr_auto_id', 'left');   
         $this->db->join('nurse', 'patient.briddd = nurse.emp_id', 'left');    
         $query = $this->db->get('patient');
         return $query->result();
     }
     ...
     function patientForConssst($qu_date) {
         $this->db->where('brdate', $qu_date);
         $this->db->join('doctor', 'patient.consultant_id = doctor.dr_auto_id', 'left');
         $query = $this->db->get('patient');
         return $query->result();
     }
   ...
   function getAppTkById($id) {
        $this->db->where('admit_p_iid', $id);
        $this->db->join('doctor', 'doctor.dr_auto_id = appointment.auto_iddd_for_dr');
   ...
   function getcon_name($cons) {
    ...
    function insert_receive_bill($bill_receive_data) {
        $this->db->insert_batch('bill_receive', $bill_receive_data);
    }
...
//Edit Receive Bill Model
   ...
   function insert_bill($data) {
        $this->db->insert_batch('create_indoor_bill', $data); 
    }

   function update_indoor_bill($p_serial, $data) {
        $this->db->where('patient_iid', $p_serial);
        $this->db->update_batch('create_indoor_bill', $data, 'bill_cat_iid');
    }
    ...
    function getBillCummsById($id) {
        $this->db->where('p_iid', $id);
        $this->db->where('coms_amount_tk !=', 0);
        $query = $this->db->get('indoor_bill_cumms');
        return $query->result();        
    }
    ...
    function update_patient_info_for_bill($p_serial, $i_data) {
        $this->db->where('p_n_id', $p_serial);
        $this->db->update('patient', $i_data);        
    }

    function discharge_patient_bed($bed_alocate_auto_iddd, $bed_discharge) {
        $this->db->where('b_a_id', $bed_alocate_auto_iddd);
        $this->db->update('bed_allocation', $bed_discharge);                
    }
    ...
    function p_end($p_id, $update_patient_data) {
        $this->db->where('p_n_id', $p_id);
        $this->db->update('patient', $update_patient_data);
    }
    ...
    function consultant_info($id) {
        $this->db->where('p_n_id', $id);
        $this->db->join('doctor', 'doctor.dr_auto_id = patient.consultant_id');
        $query = $this->db->get('patient');
        return $query->row();
    }

    function consul_sec_id($id) {
        $this->db->where('p_n_id', $id);
        $this->db->join('doctor', 'doctor.dr_auto_id = patient.consul_sec_id');
        $query = $this->db->get('patient');
        return $query->row();
    }
